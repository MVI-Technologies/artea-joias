...
export default function LotDetail({ defaultTab }) {
  ...
  const [confirmDeleteVariationId, setConfirmDeleteVariationId] = useState(null)
  ...
  const [productModalTab, setProductModalTab] = useState('principal')
      ...
      if (!romError && romaneiosData && romaneiosData.length > 0) {
        ...
        romaneiosData.forEach(rom => {
          ...
          romItems.forEach(item => {
            const rowPreco = item.valor_unitario || item.preco_unitario || item.product?.preco || 0
            grouped[clientId].items.push({
              ...
              valor_total: item.valor_total || (item.quantidade * rowPreco),
              created_at: item.created_at
            })
            grouped[clientId].total += (item.valor_total || (item.quantidade * rowPreco))
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          show_buyers_list: lotSettings.show_buyers_list
        })
        .eq('id', id)
  ...
  const saveProduct = async (keepOpen = false) => {
    ...
    try {
      ...
      const productData = {
        ...
        // preco is GENERATED, so we calculate and save custo
        custo: (() => {
          const targetPrice = parseFloat(productForm.preco);
          const margin = editingProduct?.margem_pct || 10; // Default 10% if not set
  ...
  const saveVariation = async () => {
    if (!productForm.imagem1) {
      toast.warning('O produto principal precisa ter uma imagem para usar variações!')
      return
    }
    if (!variationForm.descricao || !variationForm.preco) {
      toast.warning('Descrição e Preço são obrigatórios para a variação')
      return
    }

    setSavingProduct(true)
    try {
      const words = variationForm.descricao.trim().split(/\s+/)
      const autoGeneratedName = words.slice(0, 3).join(' ')

      const variationData = {
        nome: autoGeneratedName,
        descricao: variationForm.descricao.trim(),
        custo: (() => {
          const targetPrice = parseFloat(variationForm.preco);
          const margin = editingProduct?.margem_pct || 10;
          return targetPrice / (1 + margin / 100);
        })(),
        categoria_id: productForm.categoria_id || null,
        imagem1: productForm.imagem1,
        tipo_venda: productForm.tipo_venda,
        quantidade_pacote: productForm.tipo_venda === 'pacote' ? (parseInt(productForm.quantidade_pacote) || 12) : 12,
        ativo: true,
        qtd_minima_fornecedor: parseInt(variationForm.qtd_minima_fornecedor) || 1,
        qtd_maxima_fornecedor: productForm.qtd_maxima_fornecedor ? parseInt(productForm.qtd_maxima_fornecedor) : null,
        qtd_minima_cliente: parseInt(productForm.qtd_minima_cliente) || 1,
        multiplo_pacote: parseInt(productForm.multiplo_pacote) || 1,
        posicao_catalogo: parseInt(productForm.posicao_catalogo) || 0
      ...
      toast.success('Variação (+PRODUTO) criada com sucesso!')
      setVariationForm({ descricao: '', preco: '', qtd_minima_fornecedor: 1 })
      fetchData()
    } catch (error) {
  ...
  const deleteVariation = async () => {
    if (!confirmDeleteVariationId) return;
    const productId = confirmDeleteVariationId;
    
    try {
      // 1. Remover vinculo do lote (lot_products)
      const { error: lotError } = await supabase
        .from('lot_products')
        .delete()
      ...
      // 2. Apagar o produto em si. Como variação é produto filho que tem reservas,
      // Se der erro de fkey, será inativada.
      ...
      if (prodError) {
        console.warn('Erro ao deletar (provavelmente reservas já existem), inativando em vez de excluir.', prodError);
        await supabase.from('products').update({ ativo: false }).eq('id', productId);
      }
      ...
      console.error('Erro ao excluir variação:', e);
      toast.error('Erro ao excluir a variação. Tente novamente.');
      setConfirmDeleteVariationId(null);
    }
  }
  ...
  return (
    <div className="page-container lot-detail">
      ...
      <div className="lot-tabs">
        ...
        >
          <Factory size={16} /> Relatório Final
        </button>
      </div>
                      ...
                      <div key={itemIdx} className="reserva-item">
                        ...
                        <div className="item-info">
                          ...
                          <span className="item-qty">
                            Qtd: {item.quantidade}
                            {item.variacao && <span className="item-variation" style={{ marginLeft: 8, color: '#64748b', fontSize: '0.75rem' }}>({item.variacao})</span>}
                          </span>
                        </div>
                      ...
                      {reservaGroup.items.map(item => (
                        <label key={item.id} className="separacao-item">
                          ...
                          <div className="sep-item-info">
                            ...
                            <span className="sep-prod-qty">
                              x{item.quantidade}
                              {item.variacao && <span style={{ marginLeft: 6, color: '#64748b', fontSize: '0.75rem' }}>({item.variacao})</span>}
                            </span>
                          </div>
                        </label>
                      ))}
      ...
      {/* Tab Content: Relatório Final de Romaneios */}
      {activeTab === 'fabrica' && (
        <div className="tab-content">
      ...
      {showProductModal && (
        <div className="modal-overlay" onClick={closeProductModal}>
          <div className="modal-content product-modal-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body product-modal-body" style={{ display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'flex', borderBottom: '1px solid #e2e8f0', marginBottom: '16px', gap: '8px' }}>
                <button
                  type="button"
                  style={{ padding: '8px 16px', border: 'none', background: productModalTab === 'principal' ? '#e2e8f0' : 'transparent', fontWeight: productModalTab === 'principal' ? 'bold' : 'normal', borderRadius: '4px 4px 0 0', cursor: 'pointer' }}
                  onClick={() => setProductModalTab('principal')}
                >
                  Principal
                </button>
                <button
                  type="button"
                  style={{ padding: '8px 16px', border: 'none', background: productModalTab === 'variacoes' ? '#e2e8f0' : 'transparent', fontWeight: productModalTab === 'variacoes' ? 'bold' : 'normal', borderRadius: '4px 4px 0 0', cursor: 'pointer', color: '#2563eb' }}
                  onClick={() => setProductModalTab('variacoes')}
                >
                  +Produtos (Variações)
                </button>
              </div>

              {productModalTab === 'principal' ? (
                <>
                ...
                <div className="variations-tab-content">
                  <div style={{ marginBottom: '20px', padding: '16px', backgroundColor: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                    <h3 style={{ fontSize: '16px', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <Package size={18} /> Cadastrar ++PRODUTO (Variação da Foto)
                    </h3>
                    ...
                    <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
                      <div className="form-group" style={{ flex: '1 1 250px' }}>
                        <label>Nova Descrição * (Ex: {productForm.descricao ? productForm.descricao + ' - Tamanho 18' : 'Anel Ouro - Tamanho 18'})</label>
                        <input
                          type="text"
                          className="input-bordered"
                          value={variationForm.descricao}
                          onChange={(e) => setVariationForm({ ...variationForm, descricao: e.target.value })}
                      ...
                      <div className="form-group" style={{ width: '120px' }}>
                        ...
                        <input
                          ...
                          step="0.01"
                          className="input-bordered"
                          value={variationForm.preco}
                          onChange={(e) => setVariationForm({ ...variationForm, preco: e.target.value })}
                        />
                      </div>
                      <div className="form-group" style={{ width: '120px' }}>
                        <label>Qtd Mín.</label>
                        <input
                          type="number"
                          className="input-bordered"
                          value={variationForm.qtd_minima_fornecedor}
                          onChange={(e) => setVariationForm({ ...variationForm, qtd_minima_fornecedor: e.target.value })}
                        />
                      </div>
                      <button
                        type="button"
                        ...
                        disabled={savingProduct || !productForm.imagem1}
                      >
                  ...
                  <div className="existing-variations">
                    <h4 style={{ fontSize: '14px', fontWeight: 'bold', marginBottom: '12px', color: '#475569' }}>Produtos Relacionados (Mesma Imagem)</h4>
                    {productForm.imagem1 ? (() => {
                      const related = products.filter(lp => lp.product?.imagem1 === productForm.imagem1 && (!editingProduct || lp.product?.id !== editingProduct?.id));
                      if (related.length === 0) return <p style={{ fontSize: '13px', color: '#94a3b8' }}>Nenhuma variação registrada ainda.</p>;
                      return (
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '12px' }}>
                          {related.map(lp => (
                            <div key={lp.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '8px', border: '1px solid #e2e8f0', borderRadius: '6px', background: '#fff', position: 'relative' }}>
                              ...
                              <button 
                                type="button" 
                                onClick={() => setConfirmDeleteVariationId(lp.product.id)}
                    ...
                    })() : <p style={{ fontSize: '13px', color: '#94a3b8' }}>Nenhuma imagem definida para buscar relacionados.</p>}
                  </div>
                </div>
              )}
            </div>
      ...
      <ConfirmationModal
        isOpen={!!confirmDeleteVariationId}
        onClose={() => setConfirmDeleteVariationId(null)}
        onConfirm={deleteVariation}
        title="Excluir Variação"
        message="Tem certeza que deseja excluir esta variação? (Isso não pode ser desfeito)"
        confirmText="Excluir"
        cancelText="Cancelar"
        variant="danger"
      />
