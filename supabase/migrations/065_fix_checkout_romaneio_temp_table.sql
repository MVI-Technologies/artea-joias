-- Migration 065: Corrigir erro "cannot DROP TABLE temp_checkout_items because it is being used by active queries"
-- Ao dar RAISE EXCEPTION dentro do LOOP, o DROP explícito falha pois o cursor ainda usa a tabela.
-- Solução: não fazer DROP antes de cada RAISE; usar um único bloco EXCEPTION que sempre faz DROP IF EXISTS e re-lança.
...
COMMENT ON FUNCTION checkout_romaneio IS 'Checkout por lote. Temp table é sempre removida no bloco EXCEPTION ao ocorrer erro (ex.: estoque insuficiente).';
