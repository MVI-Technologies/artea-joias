-- Migration 059: Coluna Variação em romaneio_items
-- Permite registrar a variação escolhida por item (ex: cor ouro/prata, número do anel).

-- 1. Adicionar coluna variacao em romaneio_items
ALTER TABLE romaneio_items
ADD COLUMN IF NOT EXISTS variacao TEXT;

COMMENT ON COLUMN romaneio_items.variacao IS 'Variação escolhida pelo cliente (ex: ouro, prata, número 16)';

-- 2. Atualizar checkout_romaneio para aceitar e gravar variacao
-- Agrupa por (product_id, variacao) e insere variacao em cada linha.
...
BEGIN
    ...
    -- Agregar itens por (product_id, variacao) para suportar mesma produto com variações diferentes
    ...
    -- Validação por disponibilidade no lote (por product_id, sem distinguir variação)
...
COMMENT ON FUNCTION checkout_romaneio IS 'Checkout por lote: agrupa por product_id e variacao; grava variacao em romaneio_items';
